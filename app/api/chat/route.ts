...
export async function POST(req: NextRequest) {
	...
	const minScore = Number(process.env.MIN_SCORE || '0.25');
	...
	try {
		...
		const references: Reference[] = hasRelevant
			? items.map((it, i) => ({ label: `#${i + 1} ${it.source}`, url: `${process.env.RETRIEVER_URL}/raw?source=${encodeURIComponent(it.source)}` }))
			: [];
